...
2. **KEYWORD SEARCH FIRST** — this is the primary discovery method:
   - Search Meta Ad Library for EVERY keyword listed in targets.md Priority 4
   - Search Google Transparency for the same keywords
   - Look for ads running 30+ days (longevity = profitability)
   - Look for advertisers with multiple active ads (= serious spenders)
   - **DISCOVER new advertisers** — the known list in targets.md is just a starting point
3. Check known advertisers from the examples list in targets.md (secondary step)
4. For EVERY top advertiser found (known OR newly discovered), extract the 7 elements:
   - Headlines that hook, Price framing, Trust signals, Creative format
   - Landing page structure, Funnel stages, Emotional angle
5. **Add newly discovered advertisers** to `references/targets.md` Priority 4 list
6. Save to `~/memory/best-in-class-ads.md` — this is a LIVING document, update don't replace
7. Save detailed report to `~/drafts/ad-spy/YYYY-MM-DD-best-in-class.md`
8. Feed key findings into ad-manager briefings

**CRITICAL**: Do NOT limit yourself to the 8 known examples in targets.md. There are hundreds of advertisers in this space. Your keyword searches should discover 20-50+ advertisers per scan. If you only checked the known list — you did it wrong.
