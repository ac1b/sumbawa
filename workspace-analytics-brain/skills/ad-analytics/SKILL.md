---
name: ad-analytics
description: Pulls performance data from Meta and Google ad accounts, analyzes trends, detects problems, recommends optimizations, and learns from historical results. Produces daily/weekly reports and self-improving strategy recommendations. Also use when the user mentions "ad performance," "campaign metrics," "ad report," "CPL," "ROAS," "how are my ads doing," "ad spend analysis," or "weekly ad review." For campaign creation and management, see ad-cabinet.
metadata:
  openclaw:
    emoji: "\U0001F4CA"
    requires:
...
- `references/learnings.md` — accumulated learnings (Active Rules / Hypotheses / Archived)
...
- `~/memory/audiences.md` — audience knowledge base (update after weekly review)
- `~/memory/creatives.md` — creative performance knowledge (update after weekly review)
- `~/memory/campaigns/campaign-log.md` — campaign tracker (update CPL columns)
- `~/memory/runbook-weekly-review.md` — follow this checklist for weekly analysis
...
### After Weekly Analysis — Memory Updates (MANDATORY)

1. **`~/memory/audiences.md`** — update with audience performance data:
   - Confirmed patterns → move from Hypotheses to Active Rules
   - Disproven hypotheses → move to Archived with reason
   - New patterns → add to Hypotheses
2. **`~/memory/creatives.md`** — update with creative performance data:
   - Add winners to Winning Creatives table
   - Add failures to Failed Creatives table
   - New creative rules → add to appropriate section
3. **`~/memory/campaigns/campaign-log.md`** — update CPL 7d column
4. **`references/learnings.md`** — update all categories

### During Daily Check — Metrics Logging

When running daily check, append key metrics to the relevant `~/memory/campaigns/{SLUG}.md` Performance Log table.
